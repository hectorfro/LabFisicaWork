...
relaciona las medias de las distribuciones de $y_i$ para cada $x_i$.
 ...
 \end{itemize}
        ...
        \begin{column}{0.5\textwidth}
\vspace{0.5cm} 
3. Las cantidades $\varepsilon_i$ introduce variabilidad adicional a $y_i$ con varianza constante $\sigma^2$.
...
\end{itemize}
En el contexto del ejemplo para tener un nivel de confianza del $95 \% \alpha=0.05$. Como los grados de libertad son 19, el valor crítico $t_{\alpha / 2 \text {,dof }}$ para $\alpha / 2=0.025$ y 19 grados de libertad es de 2.093 .
 \vspace{0.4cm} 
   \begin{columns}[T] % Alineación superior de las columnas
        \begin{column}{0.5\textwidth}      
...
$$
\begin{gathered}
\mathrm{CI}=\hat{\beta}_1 \pm t_{\alpha / 2, n-2} \cdot \mathrm{SE}\left(\hat{\beta}_1\right) \\
\mathrm{CI}=-3.7234 \pm 2.093 \cdot 0.2695 \\
\mathrm{CI}=-3.7234 \pm 0.5640\\
\mathrm{CI}=[-4.2874,-3.1594]
\end{gathered}
$$
...
$$
\begin{gathered}
\mathrm{CI}=\hat{\beta}_0 \pm t_{\alpha / 2, n-2} \cdot \mathrm{SE}\left(\hat{\beta}_0\right) \\
\mathrm{CI}=2622.2834 \pm 2.093 \cdot 39.7582 \\
\mathrm{CI}=2622.2834\pm 83.2140 \\
\mathrm{CI}=[2539.0694, 2705.4975]
\end{gathered}
$$
        \end{column}
    \end{columns}
...
\item {\bf Normalidad de los Errores:}   Los errores $e_i$  deben seguir una distribución normal con media cero y varianza constante. Esto es especialmente importante para la validación de las pruebas de hipótesis y los intervalos de confianza.\end{enumerate}
