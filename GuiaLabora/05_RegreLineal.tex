...
Veamos el siguiente ejemplo, supongamos que al encargado de negocios de una tienda que fabrica y distribuye  helados le interesa  las operaciones de entrega y funcionamiento de los productos en las máquinas expendedoras de helados. Él  piensa que el tiempo que necesita un repartidor para ofrecer los productos en una máquina está relacionado con el número de cajas de producto que debe entregar, por lo tanto, decide visitar al azar 25 puntos de ventas que tienen estas máquinas expendedoras y registra el tiempo de entrega  (en minutos) y el volumen de producto entregado (en cajas) en cada uno de ellas. Las 25 observaciones se representan en el gráfico de dispersión mostrado en la figura \ref{regres1}. 
\begin{figure}[!h]
\begin{center}
\includegraphics[height=2.4in,width=2.4in]{figuras/fig18}  
\caption{Gráfico de dispersión para el tiempo y cantidad de entregas}
\label{regres1}
\end{center}
\end{figure}


Esta representación sugiere  que existe una relación entre el tiempo de entrega y el volumen de entrega,  a medida que aumenta el número de cajas aumenta el tiempo de entrega de los productos. Además, los puntos de datos caen aproximadamente a lo largo de una línea recta, existe una dispersión de los puntos. Se puede pensar que existe una relación que nos indica la tendencia de la forma
\begin{equation}
y=\beta_0+\beta_1  x \,,
\label{reglin1}
\end{equation}
donde $y$ representa los tiempos de las entregas, $x$ el volumen de entrega, 
...
\begin{figure}[!h]
\begin{center}
\includegraphics[height=2.5in,width=2.5in]{figuras/fig19}  
\caption{La  generación de observaciones en la regresión lineal.}
\label{regres2}
\end{center}
\end{figure}

Por ejemplo, si suponemos que el verdadero modelo de regresión que relaciona el tiempo de entrega con el volumen de entrega es $\mu_{y \mid x}=3,5+2 x$, y si suponemos también que la varianza es $\sigma^2=2$ entonces el resultado de estas suposiciones se puede ver figura \ref{regres2}. Aquí hemos utilizado una distribución normal para describir la variación aleatoria de $\varepsilon$. Puesto que $y$ es la suma de una constante $\beta_0+\beta_1 x$ (la media) y una variable aleatoria $\varepsilon$ de distribución normal, $y$ es una variable aleatoria de distribución normal. Para el caso en que  $x=10$ cajas, el plazo de entrega $y$ tiene una distribución normal con media $3,5+2(10)=23,5$ minutos y varianza 2. La varianza $\sigma^2$ determina la cantidad de variabilidad o ruido en las observaciones $y$ sobre el tiempo de entrega. Cuando $\sigma^2$ es pequeño, los valores observados del plazo de entrega caerán cerca de la recta, y cuando $\sigma^2$ es grande, los valores observados del plazo de entrega pueden desviarse considerablemente de la recta.
...
La ecuación (\ref{regrelineal}) puede verse como un modelo de regresión poblacional, mientras que la ecuación (\ref{diferencia}) es un modelo de regresión muestral, escrito en términos de los $n$ pares de datos $\left(y_i, x_i\right)$ $(i=1,2, \ldots, n)$. Así, el criterio de mínimos cuadrados es
...
y=m x+b \,,
$$
donde $y$ es la variable dependiente y $x$ es la variable independiente, en nuestro caso la magnitud controlada por el experimentador. Hemos llamado $b$ a $\beta_0$ y $m$ a $\beta_1$. Como se ha indicado anteriormente, los valores de esas magnitudes tendrán sus correspondientes errores, determinados por los métodos ya señalados. 
