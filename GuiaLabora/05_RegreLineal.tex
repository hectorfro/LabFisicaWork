...
Observamos que los puntos de datos no caen exactamente sobre una línea recta, esta dispersión de puntos en torno a la línea representa una variación en las notas obtenidas   y el tiempo de estudio y es de naturaleza aleatoria. Veremos que estas  relaciones estadísticas pueden ser muy útiles, aunque no tengan la exactitud de una relación funcional. Por lo tanto, la ecuación (\ref{reglin1}) debe modificarse para tener esta característica en cuenta. 
 
Así, un modelo más plausible para los datos de las notas obtenidas es
\begin{equation}
y=\beta_0+\beta_1 x+\varepsilon
\label{reglin2}
\end{equation}
donde  $\varepsilon$  es la diferencia entre el valor observado de $y$ y la recta $\left(\beta_0+\beta_1 x\right)$, y se le suele llamar el error del modelo. Es conveniente pensar en $\varepsilon$ como un error estadístico; es decir, es una variable aleatoria que explica el fracaso del modelo para ajustarse exactamente a los datos. El error puede estar compuesto por los efectos de otras variables al contabilizar las horas de estudio, errores de medición, etc. 
