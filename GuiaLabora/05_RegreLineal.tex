...
Supongamos que el encargado de negocios de una tienda de helados está interesado en estudiar  las operaciones de entrega y funcionamiento de productos en las máquinas expendedoras de helados. El encargado piensa que el tiempo que necesita un repartidor  para cargar y ofrecer el servicio en una máquina está relacionado con el número de cajas de producto que debe entregar.  El encargado visita, al azar,  25 puntos de venta que tienen estas máquinas expendedoras y registra el tiempo de entrega en la salida (en minutos) y el volumen de producto entregado (en cajas) en cada uno de ellas. Las 25 observaciones se representan en el gráfico de dispersión mostrado en la figura 1.1. 

Esta representación sugiere  que existe una relación entre el tiempo de entrega y el volumen de entrega, además da la impresión de que los puntos de datos caen generalmente, pero no exactamente, a lo largo de una línea recta:
\begin{equation}
y=\beta_0+\beta_1  x \,,
\label{reglin1}
\end{equation}
donde $y$ representa los tiempos de las entregas, $x$ el volumen de entrega, 
 $\beta_0$ es la intercepción con el eje y $\beta_1$ es la pendiente de la recta. Como se puede ver, los puntos de datos no caen exactamente sobre una línea recta, por lo que la ecuación (\ref{reglin1}) debe modificarse para tener esto en cuenta. 
 
Sea el error  $\varepsilon$ la diferencia entre el valor observado de $y$ y la recta $\left(\beta_0+\beta_1 x\right)$. Es conveniente pensar en $\varepsilon$ como un error estadístico; es decir, es una variable aleatoria que explica el fracaso del modelo para ajustarse exactamente a los datos. El error puede estar compuesto por los efectos de otras variables en los tiempos de las entregas, errores de medición, etc. Así, un modelo más plausible para los datos de los tiempos de las entregas es
\begin{equation}
