...
\item En resumen, el modelo de regresión (\ref{regrelineal}) implica que las respuestas $y_i$ proceden de distribuciones de probabilidad cuyas medias son $E\left\{y_i\right\}=\beta_0+\beta_1 x_i$ y cuyas varianzas son $\sigma^2$, iguales para todos los niveles de $x$. Además, dos respuestas cualesquiera $y_i$ y $y_j$ no están correlacionadas.
...
\label{b2}
\end{eqnarray}
donde $\bar{y} \equiv \frac{1}{n} \sum_{i=1}^n y_i$ y $\bar{x} \equiv \frac{1}{n} \sum_{i=1}^n x_i$ son las medias simples.  Nótese que los términos $\sum x_i^2$ y $\left(\sum x_i\right)^2$ no son lo mismo. Idéntica observación se debe hacer para los términos $\sum x_i y_i $ y $\sum x_i \sum y_i$.

De esta manera obtenemos la recta $\hat{y}=mx+b$ que mejor de aproxima a los puntos. Es muy común utilizar las siguiente notación:
\begin{eqnarray}
S_{x x}&=&\sum_{i=1}^n x_i^2-\frac{\left(\sum_{i=1}^n x_i\right)^2}{n}=\sum_{i=1}^n\left(x_i-\bar{x}\right)^2  \,,\\
S_{x y}&=&\sum_{i=1}^n y_i x_i-\frac{\left(\sum_{i=1}^n y_i\right)\left(\sum_{i=1}^n x_i\right)}{n}=\sum_{i=1}^n y_i\left(x_i-\bar{x}\right)
\end{eqnarray}

Esto permite escribir (\ref{eme2}) como
\begin{equation}
...
\begin{eqnarray}
\Delta m &=& \sqrt{\dfrac{n}{\Delta}} \cdot S_y 
\label{Delm}\\ \nonumber  \\
\Delta b &= &\sqrt{\dfrac{\sum\limits_{i=1}^n x_i^2}{\Delta} } \cdot S_y
\label{Delb}
\end{eqnarray}


Finalmente, para calcular $S_y$ se puede utilizar como ayuda la tabla \ref{tablamincua}.
...
\begin{example}
\label{ejemcizalla}
Un pegamento para piezas metálicas se prueba mediante la aplicación de una fuerza de cizallamiento entre las partes que han sido pegadas. La resistencia al cizallamiento de la unión entre los dos tipos de piezas es una característica crucial de calidad del pegamento. Los investigadores observan que con el transcurso del tiempo el pegamento se debilita, lo cual es un problema que los ingenieros deben resolver. 

Los datos muestran que probablemente existe una relación entre el tiempo transcurrido desde que se aplica el pegamento,  medida en semanas, y la resistencia a una fuerza de cizallamiento aplicada. Los datos se han recogido para veinte observaciones sobre la resistencia al cizallamiento y la edad del pegamento, como se muestra en la tabla \ref{datosciza}. 

Un diagrama de dispersión sugiere una fuerte relación estadística entre la resistencia al cizallamiento y la edad del pegamento. La hipótesis inicial de un modelo lineal, $y=\beta_0+\beta_1 x+\varepsilon$, parece razonable. 
\begin{figure}[h]
\begin{tabular}{ccc}
\hline Observación, $i$ & \begin{tabular}{c} 
Cizallamiento, $y_i ( \mathrm{psi} )$
\end{tabular} & \begin{tabular}{c} 
Edad del pegamento, $x_i ($ semanas $)$
\end{tabular} \\
...
plt.grid(linestyle='dotted')
plt.legend(loc='best')
plt.xlabel('Edad del pegamento (semanas)')
...
\label{sigma2}
\end{equation}
En inglés $\sigma^2=$ Error Sum of Squares=SS$_{\text{Res}}$
...
Como
\begin{equation}
\sum_{i=1}^n y_i^2-n \bar{y}^2=\sum_{i=1}^n\left(y_i-\bar{y}\right)^2 \equiv S S_{\mathrm{T}} \,,
\end{equation}

Esto no es más que la suma de cuadrados corregida de las observaciones de respuesta, por lo que
\begin{equation}
\hat{\sigma}^2=S S_{\mathrm{Res}}=S S_{\mathrm{T}}-m S_{x y}
\label{sigma2b}
\end{equation}
...
\begin{equation}
\hat{\sigma}^2=\frac{S S_{\mathrm{Res}}}{n-2}=M S_{\mathrm{Res}}
\label{sigma2c}
\end{equation}

La cantidad $M S_{\text {Res }}$ se denomina cuadrado medio residual. La raíz cuadrada de $\hat{\sigma}^2$ se llama a veces el error estándar de regresión, y tiene las mismas unidades que la variable de respuesta $y$.
...
\Delta m=\frac{9244.59}{1106.56}=2.889 \,,
$$
...
ya que si $\beta_1=0$ entonces el modelo (3.5) se reduce a $Y=\beta_0+\epsilon$, y $X$ no está asociado con $Y$. Para probar la hipótesis nula, tenemos que determinar si $\hat{\beta}_1$, nuestra estimación de $\beta_1$, es lo suficientemente lejos de cero que podemos estar seguros de que $\beta_1$ es distinto de cero. ¿Qué distancia es suficiente? Esto, por supuesto, depende de la exactitud de $\hat{\beta}_1$-es decir, depende de $\operatorname{SE}\left(\hat{\beta}_1\right)$. Si $\operatorname{SE}\left(\hat{\beta}_1\right)$ es pequeño, entonces incluso valores relativamente pequeños de $\hat{\beta}_1$ pueden proporcionar una fuerte evidencia de que $\beta_1 \neq 0$, y por lo tanto que existe una relación entre $X$ y $Y$. Por el contrario, si $\mathrm{SE}\left(\hat{\beta}_1\right)$ es grande, entonces $\hat{\beta}_1$ debe ser grande en valor absoluto para que podamos rechazar la hipótesis nula. En la práctica, calculamos un estadístico $t$, dado por
$$
